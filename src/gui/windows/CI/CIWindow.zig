...
pub var xArrowTexture: Texture = undefined;

pub fn init() void {
	arrowTexture = Texture.initFromFile("assets/cubyz/ui/inventory/crafting_arrow.png");
	xArrowTexture = Texture.initFromFile("assets/cubyz/ui/inventory/crafting_arrow_x.png");
}
...
fn canBeCrafted(recipe: *items.Recipe) bool {
...
fn findAvailableRecipes(list: *VerticalList) bool {
	...
	if (!resultItemChanged and std.mem.eql(u32, oldAmounts, itemAmount.items)) return false;
...
fn refresh() void {
	...
	const recipesChanged = findAvailableRecipes(list) or resultItemChanged;
	resultItemChanged = false;
...
pub fn openWithItem(item: BaseItemIndex) void {
	if (resultItem != null and item == resultItem.?) return;
	resultItemChanged = true;
	resultItem = item;
	if (opened) {
		refresh();
	} else {
		gui.openWindow("ci_window");
	}
}
