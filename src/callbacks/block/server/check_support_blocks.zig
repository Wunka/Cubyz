...
pub fn run(_: *@This(), params: main.callbacks.ServerBlockCallback.Params) main.callbacks.Result {
	...
	inline for (comptime std.meta.declarations(main.rotation.rotations)) |rotationMode| {
		if (params.block.mode() == main.rotation.getByID(rotationMode)) {
			if (@hasDecl(@field(main.rotation.rotations, rotationMode), "updateBlockFromNeighborConnectivity")) {
				@field(main.rotation.rotations, rotationMode).updateBlockFromNeighborConnectivity(&newBlock, neighborSupportive);
			} else {
				std.log.err("Rotation mode {s} has no updateBlockFromNeighborConnectivity function and cannot be used for {s} callback", .{rotationMode, @typeName(@This())});
			}
		}
	}
