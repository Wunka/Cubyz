...
pub fn execute(args: []const u8, source: *User) void {
	...
	const from = command.Targets.init(&split, main.globalAllocator, source) catch return;
	defer from.deinit(main.globalAllocator);
	...
	const pos = blk: {
		if (split.peek()) |arg| {
			...
			if (from.users.len != 1) {
				source.sendMessage("#ff0000Can't teleport yourself to multiple people", .{});
				return;
			}
			main.network.protocols.genericUpdate.sendTPCoordinates(source.conn, from.users[0].player.pos);
			return;
		}
	};
	if (split.next()) |_| {
